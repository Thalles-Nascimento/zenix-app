...
public class UsuarioController {
    ...
    public ResponseEntity<?> registrer(@RequestBody @Valid UsuarioRequestDTO usuarioRequestDTO, BindingResult result){
        ...
        UserDetails userRegister = usuarioService.registerUser(usuarioRequestDTO);
    ...
    public ResponseEntity<?> atualizarUsuarios(@PathVariable Long id, @RequestBody @Valid UsuarioRequestDTO user, BindingResult result) throws UsuarioException {
        if (result.hasErrors()) {
        ...
        UsuarioResponseDTO usuario = usuarioService.atualizarUsuario(id, user);
        if (usuario == null){
            Map<String, String> error = new HashMap<>();
